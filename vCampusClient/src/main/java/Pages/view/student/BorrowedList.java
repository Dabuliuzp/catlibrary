...
public class BorrowedList extends JInternalFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					BorrowedList frame = new BorrowedList();
	...
	public BorrowedList() {
		...
		Button1.setBounds(577, 377, 93, 23);
		getContentPane().add(Button1);

		Button2 = new JButton("搜索");
		Button2.setBounds(577, 298, 93, 23);
		...
		Button4.setBounds(577, 337, 93, 23);
	...
	protected void deletebutton(ActionEvent ae) {
		...
		// 弹出确认对话框，对话框的的响应
		try {
			String id = this.TextFile0.getText();// 获取书籍id
			if (JOptionPane.showConfirmDialog(this, "是否申请还书?", "正在还书...",
					JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
				out.writeObject("13");
				out.writeObject("returnMyBorrow");
